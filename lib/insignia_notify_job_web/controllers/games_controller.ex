defmodule InsigniaNotifyJobWeb.GamesController do
  use InsigniaNotifyJobWeb, :controller

  alias InsigniaNotifyJobWeb.Html.Find
  alias InsigniaNotifyJobWeb.Http.Api
  alias InsigniaNotifyJobWeb.Http.GetInsigniaData
  ...
  def get_stats_api(conn, _params) do
    {_, stats} = get_stats()
  ...
  def get_game_match_api(conn, %{"url" => url} = _params) do
    matches = get_game_matches(url)
